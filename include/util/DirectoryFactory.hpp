...
{
public:
    static std::string build_path_from_home(const std::vector<std::string>& directory_names, bool trailing_path_delim = true);
    ...
    static std::string get_schema_path(const std::string& database_name, const std::string& repository_name, bool use_test_dir = false);
private:
...
struct TestDirectoryFactory
{
    ...
    static std::string get_schema_path(const std::string& database_name, const std::string& repository_name);
};
}
}